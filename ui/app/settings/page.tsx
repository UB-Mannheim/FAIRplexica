...
const Page = () => {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push('/api/auth/signin?callbackUrl=' + encodeURIComponent('/settings'));
    }
  }, [status, router]);
  ...
  if (status === 'loading') {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <svg
          aria-hidden="true"
          className="w-8 h-8 text-light-200 animate-spin dark:text-[#202020] fill-light-secondary dark:fill-[#ffffff3b]"
          viewBox="0 0 100 101"
          fill="none"
  ...
  // Render nothing if not authenticated (the useEffect will handle redirection)
  if (status === 'unauthenticated') {
    return null;
  }
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-col space-y-4">
        <button
          ...
          className="flex items-center justify-center py-2 px-4 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors mb-8"
        >
          Log Out
        </button>
      </div>
